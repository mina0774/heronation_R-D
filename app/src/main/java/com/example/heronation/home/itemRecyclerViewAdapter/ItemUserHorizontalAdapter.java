...
public class ItemUserHorizontalAdapter extends RecyclerView.Adapter<ItemUserHorizontalAdapter.HorizontalViewHolder> {
    private ArrayList<StyleRecommendationBasedUser> ItemBasedUser;
    ...
    public ItemUserHorizontalAdapter(ArrayList<StyleRecommendationBasedUser> itemList, Context context){
        this.ItemBasedUser=itemList;
    ...
    @NonNull
    @Override
    public ItemUserHorizontalAdapter.HorizontalViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        /* 뷰홀더 객체 생성 */
        ItemUserHorizontalAdapter.HorizontalViewHolder holder=new ItemUserHorizontalAdapter.HorizontalViewHolder(view);
        return holder;
    }

    /* position에 해당하는 데이터를 뷰홀더의 아이템뷰에 표시 */
    @Override
    public void onBindViewHolder(@NonNull final ItemUserHorizontalAdapter.HorizontalViewHolder holder, int position) {
        int item_position=position;
        /* Glide를 통해 URL로 받아온 이미지를 로드해서 뷰홀더에 있는 이미지뷰에 뿌려줌 */
        Glide.with(context).load(ItemBasedUser.get(item_position).getShopImage()).error(R.drawable.shop_item_example_img_2).crossFade().into(holder.item_image);
        holder.item_name.setText(ItemBasedUser.get(item_position).getItemName());
        holder.originalPrice.setText(ItemBasedUser.get(item_position).getPrice().toString());
        ...
        holder.heart_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(!holder.isSongLikedClicked){
                    ...
                    RegisterItem(ItemBasedUser.get(holder.getAdapterPosition()).getItemId());
                }
                else {
                    // 애니메이션을 한번 실행시킨다.
                    // Custom animation speed or duration.
                    ...
                    DeleteItem(ItemBasedUser.get(holder.getAdapterPosition()).getItemId());
                }
