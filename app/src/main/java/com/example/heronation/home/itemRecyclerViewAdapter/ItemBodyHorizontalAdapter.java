...
public class ItemBodyHorizontalAdapter extends RecyclerView.Adapter<ItemBodyHorizontalAdapter.HorizontalViewHolder> {
    private ArrayList<BodyRecommendation> ItemBasedBody;
    ...
    public ItemBodyHorizontalAdapter(ArrayList<BodyRecommendation> itemList, Context context){
        this.ItemBasedBody=itemList;
    ...
    @NonNull
    @Override
    public ItemBodyHorizontalAdapter.HorizontalViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        /* 뷰홀더 객체 생성 */
        ItemBodyHorizontalAdapter.HorizontalViewHolder holder=new ItemBodyHorizontalAdapter.HorizontalViewHolder(view);
        return holder;
    }

    /* position에 해당하는 데이터를 뷰홀더의 아이템뷰에 표시 */
    @Override
    public void onBindViewHolder(@NonNull final ItemBodyHorizontalAdapter.HorizontalViewHolder holder, int position) {
        int item_position=position;
        /* Glide를 통해 URL로 받아온 이미지를 로드해서 뷰홀더에 있는 이미지뷰에 뿌려줌 */
        Glide.with(context).load(ItemBasedBody.get(item_position).getShopImage()).error(R.drawable.shop_item_example_img_2).crossFade().into(holder.item_image);
        holder.item_name.setText(ItemBasedBody.get(item_position).getItemName());
        holder.originalPrice.setText(ItemBasedBody.get(item_position).getPrice().toString());
        holder.heart_button.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View view) {
                if(!holder.isSongLikedClicked){
                    ...
                    RegisterItem(ItemBasedBody.get(holder.getAdapterPosition()).getItemId());
                }
                else {
                    // 애니메이션을 한번 실행시킨다.
                    // Custom animation speed or duration.
                    ...
                    DeleteItem(ItemBasedBody.get(holder.getAdapterPosition()).getItemId());
                }
